clear;
%Example demonstration of the ranodm initialization for overlapping communoty detection
...
%res_path_TA = 'Reddit26_res_TA(over_rand).txt'; %Path to save the (overlapping community detection) result of DHCD T-A
%res_path_AT = 'Reddit26_res_AT(over_rand).txt'; %Path to save the (overlapping community detection) reuslt of DHCD A-T
%==========
adj = load('data/Reddit25_adj.txt');
att = load('data/Reddit25_node_attri.txt');
gnd_path = 'data/Reddit25_gnd.txt';
res_path_TA = 'Reddit25_res_TA(over_rand).txt';
res_path_AT = 'Reddie25_res_AT(over_rand).txt';
...
for l=1:num_params
	...
	fid = fopen('DHCD_T-A(over_rand).txt', 'at');
	fprintf(fid, 'T-A Ch. Alpha: %8.4f; Obj: %8.4f; F-Score: %8.4f; Jaccard: %8.4f\n', [alpha, obj, Fsc, Jac]);
	fclose(fid);
	%====================
    %Independently run the DHCD T-A algorihtm multiple times
    for t=2:num_runs
        [topo_mem_init, ~] = NMF_rand_init(adj, num_topo_clus);
        [~, att_desc_init] = NMF_rand_init(att, num_att_clus);
        trans_TA_init = rand(num_topo_clus, num_att_clus);
	...
	fid = fopen('DHCD_A-T(over_rand).txt', 'at');
    ...
    for t=2:num_runs
        [att_mem_init, att_desc_init] = NMF_rand_init(att, num_att_clus);
        trans_AT_init = rand(num_att_clus, num_topo_clus);
